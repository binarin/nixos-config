...
* TODO Drop fj-comments.sh when forgejo-cli adds structured output
  Wrapper script for fetching issue comments with metadata (id, timestamps).
  Location: .claude/skills/fj-issues/fj-comments.sh
  When forgejo-cli supports JSON output for comments, this can be removed.
...
* TODO smb-sketchup like share for valak vms
  ...
  :END:
* DONE Script to rolling upgrade all deploy_rs machines
  :PROPERTIES:
  ...
  :END:
* DONE nixos-secrets - Python Secrets Manager
** Overview
...
age-keygen -o secrets/claude-nixos-config/user-binarin-age > secrets/claude-nixos-config/user-binarin-age.pub
sops encrypt -i secrets/claude-nixos-config/user-binarin-age

add 2 keys in .sops.yaml from secrets/claude-nixos-config/ssh_host_ed25519_key.pub and secrets/claude-nixos-config/user-binarin-age.pub
add secrets.yaml / user-binarin.yaml stanzas in .sops.yaml
add empty yamls ({}) secrets.yaml / user-binarin.yaml
