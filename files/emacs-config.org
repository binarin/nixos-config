...
** use-package
   #+begin_src emacs-lisp :tangle yes
     (setopt use-package-ensure-function #'(lambda (&rest _) t))
   #+end_src
** Common folders
   #+begin_src emacs-lisp :tangle yes
     (eval-and-compile
...
** No littering
   #+begin_src emacs-lisp :tangle yes
     ...
     ;; looks like a sandboxed nix build
...
** Search
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     (defun binarin/search-words ()
       ...
       (when (use-region-p)
         (browse-url
          (concat "https://kagi.com/search?q="
...
*** Prefer Russian and UTF-8
    #+begin_src emacs-lisp :tangle yes
      ...
      (defun binarin/change-window-divider ()
        (awhen (or buffer-display-table standard-display-table)
          (set-display-table-slot it 5 ?┃)
          (set-window-display-table (selected-window) it)))

      (add-hook 'window-configuration-change-hook 'binarin/change-window-divider)
    #+end_src
...
** Folding
   #+begin_src emacs-lisp :tangle no
     (use-package kirigami
       :ensure t
       :bind
       (("C-c k o"   . 'kirigami-open-fold)     ; Open fold at point
        ("C-c k O"   . 'kirigami-open-fold-rec) ; Open fold recursively
        ("C-c k m"   . 'kirigami-close-folds)   ; Close all folds
        ("C-c k c"   . 'kirigami-close-fold)    ; Close fold at point
        ("C-c k r"   . 'kirigami-open-folds)    ; Open all folds
        ("C-c k TAB" . 'kirigami-toggle-fold)) ; Toggle fold at point
       )
   #+end_src
   ...
   #+begin_src emacs-lisp :tangle yes
     ...
     (use-package lsp-mode
       ...
       :bind (:map lsp-mode-map
                   ("<f22>" . lsp-execute-code-action)
                   ("C-c l d" . lsp-rust-analyzer-open-external-docs))
       :commands (lsp lsp-deferred)
       :config
       (add-hook 'lsp-mode-hook 'lsp-enable-which-key-integration))

     (defun binarin/set-lsp-ui-doc-frame-font (frame _w)
     ...
     (use-package lsp-ui
       ...
       :defer t
       :config
       (add-hook 'lsp-ui-doc-frame-hook 'binarin/set-lsp-ui-doc-frame-font)
...
** Compilation
   ...
   #+begin_src emacs-lisp :tangle yes
     ...
     ;; XXX What I really want is to skip warnings only when there are errors
     (setf compilation-skip-threshold 2
     ...
     (defun binarin/toggle-compilation-warning-recognition ()
       (interactive)
       (setf compilation-skip-threshold
             (if (eql compilation-skip-threshold 0)
                 (progn
                   (message "`next-error` will ignore warnings")
                   2)
               (message "`next-error` will recognize warnings")
               0)))
...
** AI coding
   XXX broken as of 2026-02-12
   #+begin_src emacs-lisp :tangle no
     (use-package agent-shell
       :ensure t
       :defer 30
       :commands (agent-shell agent-shell-make-environment-variables agent-shell-anthropic-make-authentication)
       :config
       (setf agent-shell-anthropic-claude-environment (agent-shell-make-environment-variables :inherit-env t))
       (setf agent-shell-anthropic-authentication (agent-shell-anthropic-make-authentication :login t)))
   ...
   #+begin_src emacs-lisp :tangle yes
     (use-package gptel
       :ensure t
       :bind (("C-M-S-s-<return>" . gptel-send))
       :config
       (setf gptel-backend (gptel-make-gemini "Gemini"
...
** Rust
   #+begin_src emacs-lisp :tangle yes
     (use-package rustic
       ...
       :bind
       (:map rustic-mode-map
             ("<f16>" . rustic-cargo-run)
             ("<f17>" . rustic-cargo-test))
       :config
       ;; (add-hook 'eglot-managed-mode-hook (lambda () (flymake-mode -1)))
       (add-hook 'rustic-mode 'rustic-doc-mode)
       (setf rustic-compilation-panic
             (cons (rx (: "thread '" (+ (not "'")) "'" (+? not-newline) " panicked at ")
                       (group (+ not-newline))
                       ":"
                       (group (+ (any digit)))
                       ":"
   ...
   #+begin_src emacs-lisp :tangle no
     ...
     (defun binarin/rust-mode-hook ()
       (add-hook 'eldoc-documentation-functions 'flymake-eldoc-function -100 t)
       (setq-local eldoc-documentation-strategy #'eldoc-documentation-compose)
       (eglot-ensure)
       (add-hook 'before-save-hook 'eglot-format-buffer nil t))

     (add-hook 'rust-mode-hook 'binarin/rust-mode-hook)
   #+end_src
...
** Go
   #+begin_src emacs-lisp :tangle yes
     (use-package dape
       ...
       (add-hook 'dape-start-hook 'binarin/save-some-buffers)

       ;; To not display info and/or buffers on startup
       ;; (remove-hook 'dape-start-hook 'dape-info)
       (remove-hook 'dape-start-hook 'dape-repl))

     (defun binarin/save-some-buffers ()
     ...
     (use-package go-mode
       :ensure t
       :mode "\\.go\\'"
       :hook (go-mode . binarin/go-mode-hook)
     ...
     (use-package gotest
       ...
       :bind (:map go-mode-map
                   ("C-c t f" . binarin/go-tests)
                   ("<f17>" . binarin/go-tests)
   ...
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (use-package org
       ...
       (
        ...
        ("C-c r" . org-capture)
        :map org-mode-map
        ("<Open>" . org-edit-special)
        :map org-src-mode-map
        ("<Open>" . org-edit-src-exit))
       :config
         ...
         (when (markerp marker)
           (org-with-point-at marker
             (awhen (and (= 3 (org-current-level)) (org-up-heading-safe)
               ...
               ;; Extract priority cookie with its face properties (if present)
               (let ((priority-cookie nil)
                     (priority-regexp (rx "[#" (any "A-D") "]")))
                 ;; Find and extract priority cookie preserving text properties
                 (when (string-match priority-regexp item)
                   (setq priority-cookie (match-string 0 item))
                   ;; Remove priority cookie from original position
                   (setf item (replace-regexp-in-string
                               (rx (+ space) "[#" (any "A-D") "]")
                               ""
                               item nil nil)))
                 ;; Build the replacement string: priority + project name
                 (let ((project-text (propertize (s-concat " [" it "] ")
                                                 'face 'org-agenda-dimmed-todo-face)))
                   (setf item (replace-regexp-in-string
                               (rx (or "NEXT" "WAIT") (group anychar) (* anychar) eos)
                               (if priority-cookie
                                   (concat " " priority-cookie project-text)
                                 project-text)
                               item nil nil 1)))))))
         item))
     ...
     (setq org-agenda-custom-commands '(("a" "Agenda and NEXTs" ((agenda "")
                                                                 (tags-todo "-agenda_hide+TODO=\"NEXT\""
                                                                            ((org-agenda-overriding-header
                                                                              (lambda () (concat
                                                                                          (propertize "Tasks to do " 'face 'org-agenda-structure)
                                                                                          (propertize "NEXT" 'face '(:foreground "#93E0E3" :weight bold))
...
** Caldav sync
   #+begin_src emacs-lisp :tangle yes
     (use-package org-caldav
       ...
       :custom
       (org-caldav-url "https://nextcloud.lynx-lizard.ts.net/remote.php/dav/calendars/binarin")
...
** [[https://kapeli.com/dash][Dash for macOS - API Documentation Browser, Snippet Manager - Kapeli]]
 :PROPERTIES:
  ...
  :END:
  [2024-11-24 Sun 09:15]
** [[https://sr.ht/~niklaseklund/detached.el/][detached.el: An Emacs package to launch, and manage, detached processes]]
   :PROPERTIES:
   :ID:       973d5ad6-b736-4d32-90b6-dbebe839baa6
   :CREATED:  [2026-01-07 Wed 18:06]
   :END:
** Devdocs
