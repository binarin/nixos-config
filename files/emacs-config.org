...
** No toolbars/scrollbars
   #+BEGIN_SRC emacs-lisp :tangle yes
     ...
     (unless (and (eq binarin/system-type 'darwin)
                (window-system))
         (menu-bar-mode -1))
...
** Meow
   #+begin_src emacs-lisp :tangle yes
     ...
     (use-package meow
       :functions meow-motion-define-key meow-normal-define-key meow-leader-define-key meow-define-keys meow-register-state
       meow--set-cursor-color meow--set-cursor-type meow-update-display meow--disable-current-state
     ...
     (defvar meow-org-motion-keymap (make-keymap))
     ...
     (defun binarin/maybe-meow-org-motion-mode ()
       (interactive)
       (when (eq major-mode 'org-mode)
         (meow-org-motion-mode)))

     (meow-define-keys 'normal
       '("O" . binarin/maybe-meow-org-motion-mode))
     ...
     (setf meow-replace-state-name-list
           '((org-motion . "ORG")
             (normal . "NRM")
             (motion . "MOT")
             (keypad . "KPD")
     ...
     (with-no-warnings ;; unused lexicals from zenburn-with-color-variables
       (zenburn-with-color-variables
         (ignore class)
         (defface telephone-line-meow-org-motion
           `((t (:background ,zenburn-orange :inherit telephone-line-evil)))
           "Face for my custom org-motion meow state"
           :group 'telephone-line-evil)
         (set-face-attribute 'telephone-line-evil-normal nil :background zenburn-green-4)
         (set-face-attribute 'telephone-line-evil-insert nil :background zenburn-red-5)
         (set-face-attribute 'telephone-line-evil-emacs nil :background zenburn-magenta)
         (set-face-attribute 'telephone-line-evil-operator nil :background zenburn-blue-5)
         (set-face-attribute 'telephone-line-evil-replace nil :background zenburn-yellow)))

     (defun binarin/telephone-line-modal-face (orig-fun active)
       (ignore orig-fun)
       (cond ((not active) 'mode-line-inactive)
             ...
             (meow-org-motion-mode 'telephone-line-meow-org-motion)))

     (telephone-line-defsegment* binarin/meow-state-segment ()
       (when (bound-and-true-p meow-mode)
         (ignore face)
         (s-trim (meow-indicator))))

     (add-to-list 'telephone-line-lhs '(evil binarin/meow-state-segment))

     (advice-add 'telephone-line-modal-face :around 'binarin/telephone-line-modal-face)
   #+end_src
** SSH agent
   #+begin_src emacs-lisp :tangle yes
   ...
   #+begin_src emacs-lisp :tangle yes
     ...
     (use-package consult
       ...
       (("C-c M-x"   . consult-mode-command)
        ...
        ("C-x C-b"   . consult-buffer)              ;; So that I can do `SPC x b` in meow
   ...
   #+begin_src emacs-lisp :tangle yes
     ...
     (defvar binarin/direnv-boring-items
       '("AR" "AS" "CC" "CONFIG_SHELL" "CXX" "DEVENV_DOTFILE" "DEVENV_STATE" "HOST_PATH" "IN_NIX_SHELL" "LD"
         ...
         "DEVELOPER_DIR" "LD_DYLD_PATH" "MACOSX_DEPLOYMENT_TARGET" "NIX_APPLE_SDK_VERSION"
         "NIX_BINTOOLS_WRAPPER_TARGET_HOST_arm64_apple_darwin" "NIX_CC_WRAPPER_TARGET_HOST_arm64_apple_darwin" "NIX_DONT_SET_RPATH"
         "NIX_DONT_SET_RPATH_FOR_BUILD" "NIX_IGNORE_LD_THROUGH_GCC" "NIX_NO_SELF_RPATH" "PATH_LOCALE" "SDKROOT" "ZERO_AR_DATE"
         "__darwinAllowLocalNetworking" "__impureHostDeps" "__propagatedImpureHostDeps" "__propagatedSandboxProfile"
         "__sandboxProfile" "XPC_SERVICE_NAME"
         ))
...
** ledger/hledger
 #+begin_src emacs-lisp :tangle yes
   ...
   (cl-defstruct resolve-scope trn date use-date amount use-amount regex use-regex rules-file)

   (defun binarin/resolve-ledger-transaction ()
     (interactive)
     (let ((scope (transient-scope)))
       (message "%s" (resolve-scope-trn scope))))
   ...
   (transient-define-prefix binarin/ledger-resolve ()
     ...
     [:description (lambda () (format "Date: %s" (resolve-trn-date (resolve-scope-trn (transient-scope)))))
                   ""]
     ["Actions\n"
      ("g" binarin/resolve-ledger-transaction :description "Resolve to global")]
     (interactive)
     (let ((trn (binarin/get-resolve-trn)))
       (with-slots (date payee amount account) trn
         (transient-setup 'binarin/ledger-resolve nil nil
                          :scope (make-resolve-scope :trn trn :date date :use-date nil :amount amount :use-amount nil :regex payee :use-regex nil :rules-file nil)))))
 #+end_src
* Major modes
** pdf-tools
   #+begin_src emacs-lisp :tangle yes
