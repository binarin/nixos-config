#+PROPERTY: header-args:emacs-lisp :lexical yes :results value pp silent
* File header
** Directories
   #+begin_src emacs-lisp :tangle yes
     (eval-and-compile
       ...
       (setq package-user-dir (cache-file-name "elpa/")
             no-littering-etc-directory (cache-file-name "no-littering" "config")
             no-littering-var-directory (cache-file-name "no-littering" "data")
             native-comp-async-env-modifier-form `(setq package-directory-list ',package-directory-list)))

     ;; some stuff inside `eval-and-compile` is because of:
     ;; no-littering-XXX - https://github.com/emacscollective/no-littering/issues/249
     ;; native-comp-async-env-modifier-form - of https://github.com/NixOS/nixpkgs/issues/145302#issuecomment-2506666795
