...
- **User list is single-sourced** in `lib/xray/config-lib.nix` as `userNames = [ "xr1" "xr2" ]`.
- **Never import `self.nixosModules.disko`** for these machines. It reads `specialArgs.inventoryHostName`, which is present only in the flake-output `extendModules` wrapper and ABSENT during clan's own machine eval (what `clan vars` uses) → throws. Import the per-host `disko.nix` by explicit `"${self}/my-machines/<host>/disko.nix"` path (or inline the layout / use the `disko-template-*` modules as `acme` does). disko is activated globally by clan-core, so no `inputs.disko.nixosModules.disko` import is needed.
- **Guest-VLAN machines need a `clan-hosts` guard.** `modules/clan.nix`'s `clan-hosts` builds `/etc/hosts` by doing `ipAllocation.<name>.home.primary.address` for **every** machine; a guest-only machine (`xray-exit`, no `.home` allocation) makes it throw for all machines. Guard the map with `... && flakeConfig.inventory.ipAllocation."${name}" ? home` (already applied to `modules/clan.nix` in this branch).
- **Verification is parse-level only in this environment.** Do NOT run `nix eval .#nixosConfigurations.<m>...` / `.drvPath` — evaluating a full clan `nixosConfiguration` instantiates the entire system derivation and consumes 16 GB+. Use `nix-instantiate --parse <file>` for syntax and leave full build/eval to the operator (who owns deploys).

## File Structure

- `lib/xray/config-lib.nix` — pure, lib-free attrset of builder functions + `userNames`. Single source of truth for xray JSON structure. **Lives under `lib/` (not `modules/`)** because `flake.nix` does `import-tree ./modules`, which would try to evaluate any `.nix` under `modules/` as a flake-parts module — and this file is a bare attrset, not a module. `lib/` is imported explicitly, never tree-imported.
- `lib/xray/config-lib-test.nix` — pure `nix eval` test of the builders (co-located with config-lib so its relative `import ./config-lib.nix` resolves).
- `modules/xray/shared.nix` — `flake.nixosModules.xray-shared`: the shared clan-var generators (`xray-reality-front`, `xray-reality-exit`, `xray-link`, `xray-front-params`, `xray-exit-params`, `xray-geo`). (A real flake-parts module — correctly tree-imported.)
...
**Files:**
- Create: `lib/xray/config-lib.nix`
- Test: `lib/xray/config-lib-test.nix`

IMPORTANT: these two files go under `lib/`, NOT `modules/`. `flake.nix` runs `import-tree ./modules` and would try to load anything under `modules/` as a flake-parts module; `config-lib.nix` is a bare attrset of functions, so it must live outside the tree-imported directory (mirrors the existing `lib/networks-lookup.nix` helper).

**Interfaces:**
- Produces: `import "${self}/lib/xray/config-lib.nix"` → attrset with:
  - `userNames :: [string]`
...
Create `lib/xray/config-lib-test.nix`:
...
Run: `cd /home/binarin/personal-workspace/nixos-config && nix eval --file lib/xray/config-lib-test.nix`
...
Run: `nix eval --file lib/xray/config-lib-test.nix`
Expected: `"ok"`

- [ ] **Step 5: Format, lint, commit**

```bash
nix fmt lib/xray/config-lib.nix lib/xray/config-lib-test.nix
just lint
git add lib/xray/config-lib.nix lib/xray/config-lib-test.nix
...
- [ ] **Step 2: Stage and parse-check (NO full eval)**

```bash
git add modules/machines/xray-exit.nix
nix-instantiate --parse modules/machines/xray-exit.nix >/dev/null && echo "parse OK"
```
Expected: `parse OK`. Do NOT run `nix eval .#nixosConfigurations.xray-exit...` — a full clan `nixosConfiguration` eval instantiates the entire system derivation and consumes 16 GB+. The rendered-config shape (`8443`/`freedom`), the guest-VLAN `mkForce` override, `services.tailscale.enable = false`, and disko all get validated by the **operator's** build (`clan machines update xray-exit`), not here. Confirm by inspection that the module matches this task's code.
...
- [ ] **Step 3: Stage and parse-check (NO full eval)**

```bash
git add modules/machines/xray-front.nix my-machines/xray-front/disko.nix
nix-instantiate --parse modules/machines/xray-front.nix >/dev/null && echo "front parse OK"
nix-instantiate --parse my-machines/xray-front/disko.nix >/dev/null && echo "disko parse OK"
```
Expected: `front parse OK` / `disko parse OK`. Do NOT run `nix eval .#nixosConfigurations.xray-front...` (16 GB+ full-system instantiation). The per-user generators (`files = client.json,uuid`; `dependencies = xray-front-params,xray-reality-front,xray-geo`), the two-client inbound chaining to `to-exit` on :443, and disko all get validated by the operator's `clan machines update xray-front` build. Confirm by inspection against this task's code.
...
Assistant-runnable (cheap):
- [ ] `nix eval --file lib/xray/config-lib-test.nix` → `"ok"`
- [ ] `nix-instantiate --parse` clean on both machine modules + `my-machines/xray-front/disko.nix`
- [ ] `nix eval .#nixosModules.xray-shared --apply 'm: "ok"'` → `"ok"` (light — module import only)
- [ ] `nix build .#packages.x86_64-linux.xray-share` → success
- [ ] `nix fmt` clean, `just lint` clean
- [ ] Operator runbook exists and is committed

Operator-run (heavy — NOT in this environment; 16 GB+ full-system eval each):
- [ ] `clan machines update xray-exit` builds and deploys
- [ ] `clan machines update xray-front --target-host root@<ec2>` builds and deploys
- [ ] Live end-to-end per Task 7 (residential-IP exit, split-tunnel, REALITY handshake)
```
