...
This document describes the workflow for working on project issues and pull
requests in Forgejo.

## Important Notes

**Primary place for storing planning information/questions is forgejo issue
...
tracking. When working on branches that track `claude-staging` (the fork), you
must specify the upstream repo explicitly for issues.

Always use the full repo reference: `binarin/nixos-config#<ID>`

## Git state

Always run `git fetch origin` before making any decisions based on git state
(branch comparisons, checking what's merged, deciding on fixup vs new commit,
etc.). Local refs can be stale, leading to incorrect assumptions about what
commits exist upstream.
...
Only edit the plan comment (first claude-nixos-config comment), never edit
feedback comments.
...
The plan scope is only the implementation, no deployments at all - if you can't
test something just from the confines of repository, skip including it from
plan altogether. The ultimate checks are `eval-all` and `build-all`.
...
Claude agent does not have write access to `origin` remote. Instead, push to
`claude-staging` remote (fork repository at `claude-nixos-config/nixos-config`).
...
To update an existing PR, just push new commits to the same branch on
`claude-staging`:
...
- If you need to add some sops secrets, don't bother getting the real values -
  just stick random placeholders there.
