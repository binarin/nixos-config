...
let
  ...
  getAttrDefault =
    name: default: attrs:
    if hasAttr name attrs then getAttr name attrs else default;

  allMappings = import "${self}/inventory/priv-pam-u2f-keys.nix";

  relevantMappings =
    filterOrigin: filterAppId:
    lib.filter ({ origin, appid, ... }: origin == filterOrigin && appid == filterAppId);

  appendMappingToUser =
    mapping: username: acc:
    acc // { "${username}" = (getAttrDefault username [ ] acc) ++ [ mapping ]; };
