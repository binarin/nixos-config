...
let
  cfg = config.services.rabbitmq-broker;

  inherit (builtins) isAttrs concatStringsSep;
  inherit (lib) mapAttrsToList;

  recurse = path: attrs:
    let elementHandler = name: value:
      let prePath = if isNull path
                      then ""
                      else "${path}.";
      in if isAttrs value
           then recurse ("${prePath}${name}") value
           else "${prePath}${name} = ${toString value}";
    in mapAttrsToList elementHandler attrs;

  attrsToNewConfig = attrs: concatStringsSep "\n" (lib.flatten (recurse null attrs));

  config_file = pkgs.writeText "rabbitmq.config" (attrsToNewConfig cfg.config);
...
in {
  ...
  options = {
    services.rabbitmq-broker = {
      ...
      config = mkOption {
        ...
        type = types.attrs;
        description = ''
          XXX Verbatim configuration file contents.
  ...
  config = mkIf cfg.enable {
    environment.etc."rabbitmq.conf".source = config_file;
