...
let
  ...
  networks = let
    ...
    regularNixFiles = lib.attrNames (lib.filterAttrs (n: v: v == "regular" && lib.hasSuffix ".nix" n) dir);
    val = lib.map (fn:
      let
  ...
  in lib.listToAttrs val;

  expandIpAllocationTarget = hostNameWithTags:
    if lib.isString hostNameWithTags
    then [ hostNameWithTags "primary" ]
    else hostNameWithTags;

  assignIp = ipAllocationTarget: ip:
    let
      hostName = lib.head ipAllocationTarget;
      tags = lib.tail ipAllocationTarget;
    in {
      "${hostName}" = lib.genAttrs tags (tag: {_type = "leaf"; address = ip; });
    };

  allAssignmentsForNetwork = ipam: lib.map ({name, value}:
    let
      ip = name;
      allocationTarget = expandIpAllocationTarget value;
    in assignIp allocationTarget ip
  ...
  in lib.zipAttrsWith (hostName: networkTags: lib.foldr (a: b: a // b) {} networkTags) perNetwork;
...
{ # allAssignmentsForNetwork networks.home.ipam'
  options = {
    ...
    inventory.ipAllocation = lib.mapAttrsRecursiveCond (as: !(as ? "_type")) (path: x:
        let
    ...
    inventory.networks = lib.mapAttrs (netName: {info, ...}:
