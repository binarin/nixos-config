...
in
{
  ...
  lib.style.template = name: template: data: pkgs.stdenv.mkDerivation {
    name = "${name}";
    ...
    jsonData = builtins.toJSON {
      ...
      fonts = { inherit (config.stylix.fonts) sizes; } // lib.genAttrs ["serif" "sansSerif" "monospace" "emoji"] (nm:
        config.stylix.fonts."${nm}".name
      );
    };
