{ self, ... }:
{
  nixosSharedModules = [ self.nixosModules.pam-u2f ];

  flake.nixosModules.pam-u2f =
    {
      lib,
      pkgs,
      config,
    ...
    in
    {
      key = "nixos-config.modules.nixos.pam-u2f";
      ...
      config =
        let
          enabledSvcs = lib.filterAttrs (_n: o: o.enable) cfg.services;
        in
        {

          environment.etc = lib.mapAttrs' (service: _opts: {
