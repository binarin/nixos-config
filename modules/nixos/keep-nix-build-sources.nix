{
  ...
  in
    if builtins.isAttrs fl && fl ? "inputs"
    then (lib.mapAttrs (name: value: walkInputs value) fl.inputs) // selfAttrs
    else selfAttrs;

  flakeInputsRecursive = fl:
    ...
    pkgs.writeShellScriptBin "flake-all-sources-keeper" ''
      ...
      ${lib.concatStringsSep "\n" (
        lib.map ({
          ...
          source,
          ...
        }: "${filename}: ${source}") (allInputs flake)
      )}
