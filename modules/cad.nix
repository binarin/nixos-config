...
}:
{
  flake.homeModules.cad =
    ...
    let
      freecad-dpi-wrapper = pkgs.runCommand "FreeCAD-dpi-aware" { } ''
    ...
    in
    {
      key = "nixos-config.modules.home.cad";
  ...
  flake.nixosModules.cad =
    { config, ... }:
    {
      key = "nixos-config.modules.nixos.cad";

      config.home-manager.sharedModules = [ self.homeModules.cad ];
    };

  nixosSharedModules = [ self.nixosModules.cad ];
}
